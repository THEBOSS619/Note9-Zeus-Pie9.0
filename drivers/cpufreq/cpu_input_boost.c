...
static void set_stune_boost(struct boost_drv *b, u32 state, u32 bit, char *st, int level,
			    int *slot)
{
	if (level && !(state & bit)) {
...
static void input_boost_worker(struct work_struct *work)
{
	...
	set_stune_boost(b, state, INPUT_STUNE_BOOST, ST_TA, input_stune_boost,
		&b->input_stune_slot);
}
...
static void max_boost_worker(struct work_struct *work)
{
	...
	set_stune_boost(b, state, MAX_STUNE_BOOST, ST_TA, max_stune_boost,
		&b->max_stune_slot);
}
			  ...
			  unsigned long action, void *data)
{
	...
	if (*blank == FB_BLANK_UNBLANK) {
		...
		set_stune_boost(b, state, DISPLAY_STUNE_BOOST, ST_TA,
			           display_stune_boost, &b->display_stune_slot);
		set_stune_boost(b, state, DISPLAY_BG_STUNE_BOOST, ST_BG,
